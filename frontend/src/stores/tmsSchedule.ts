...
export const useTmsScheduleStore = defineStore('tmsSchedule', () => {
    ...
    const metadata = ref<{ name: string, type: string, lastModified: number, uploadedDate: number, size: number } | {}>({});
    ...
    async function loadFromJson(json: object) {
        return new Promise<void>((resolve, reject) => {
            ...
            const data = json as { timetable: Show[], metadata: { name: string, type: string, lastModified: number, uploadedDate: number, size: number } };

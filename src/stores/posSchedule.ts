...
export const usePosScheduleStore = defineStore('posSchedule', () => {
    ...
    const metadata = ref<{ name: string, type: string, lastModified: number, size: number, uploadedDate: number, timestamp: number } | {}>({});
    ...
    async function uploadXml(fileList: File[] | FileList) {
        ...
        metadata.value = {
            ...
            timestamp: xml?.schedule?._attributes?.timestamp || 0,
        };
    }
